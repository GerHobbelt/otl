...
//                          // the LONG BYTE to be written.
  
  lob<<f2; // writing first chunk of the LONG BYTE into lob


  f2[2122]='?';
  f2.set_len(2123); // setting the size of the second chunk

  lob<<f2; // writing the second chunk of the LONG BYTE into lob
...
//                          // the LONG BYTE to be written.
  ...
  lob2<<f2; // writing first chunk of the LONG BYTE into lob

  f2[2122]='?';
  f2.set_len(2123); // setting the size of the second chunk

  lob2<<f2; // writing the second chunk of the LONG BYTE into lob
...
{ 
 ...
 i.open(1, // buffer size. To read LONG BYTEs, it should be set to 1
 ...
 int f1=0;
 otl_lob_stream lob; // Stream for reading LONG BYTE
 otl_lob_stream lob2; // Stream for reading LONG BYTE
 ...
 while(!i.eof()){ // while not end-of-data
  i>>f1;
  cout<<"f1="<<f1<<endl;
  i>>lob; // initializing LOB stream by reading the LONG BYTE reference 
          // into the otl_lob_stream from the otl_stream.
  i>>lob2; // initializing LOB stream by reading the LONG BYTE reference 
          // into the otl_lob_stream from the otl_stream.
  int n=0;
  while(!lob.eof()){ // read while not "end-of-file" -- end of LONG BYTE
   ++n;
   lob>>f2; // reading a chunk of LONG BYTE
   cout<<"   chunk #"<<n;
   cout<<", f2="<<f2[0]<<f2[f2.len()-1]<<", len="<<f2.len()<<endl;
  }
  lob.close(); // closing the otl_lob_stream.
  n=0;
  while(!lob2.eof()){ // read while not "end-of-file" -- end of LONG BYTE
   ++n;
   lob2>>f2; // reading a chunk of LONG BYTE
   cout<<"   chunk #"<<n;
...
int main()
{
 ...
 try{
  ...
  otl_cursor::direct_exec
   (
    db,
    "create table test_tab(f1 int, f2 long byte, f3 long byte)"
    );  // create table
