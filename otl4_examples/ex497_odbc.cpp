...
{otl_long_string f2(6000); // define long string variable
 ...
 for(i=1;i<=20;++i){
  ...
  lob.set_len(5001+2123); // setting the total  size of
                          // the VARCHAR(MAX) value to be written.
  
  lob<<f2; // writing first chunk of the VARCHAR(MAX) value into lob


  f2[2122]='?';
  f2.set_len(2123); // setting the size of the second chunk

  lob<<f2; // writing the second chunk of the VARCHAR(MAX) value into lob
  ...
  lob2<<f2; // writing first chunk of the VARCHAR(MAX) value into lob

  f2[2122]='?';
  f2.set_len(2123); // setting the size of the second chunk

  lob2<<f2; // writing the second chunk of the VARCHAR(MAX) value into lob
...
// insert rows in table
{
 ...
 lob.set_len(6001*4); // setting the total size of of the VARCHAR(MAX) value to be written
 for(int i=1;i<=4;++i)
  lob<<f2; // writing chunks of the VARCHAR(MAX) value into the otl_lob_stream
...
{ 
 ...
 i.open(1, // buffer size. To read VARCHAR(MAX) values, it should be set to 1
 ...
 int f1;
 otl_lob_stream lob; // Stream for reading VARCHAR(MAX) value
 otl_lob_stream lob2; // Stream for reading VARCHAR(MAX) value
 ...
 while(!i.eof()){ // while not end-of-data
  i>>f1;
  cout<<"f1="<<f1<<endl;
  i>>lob; // initializing LOB stream by reading the VARCHAR(MAX) value reference 
          // into the otl_lob_stream from the otl_stream.
  i>>lob2; // initializing LOB stream by reading the VARCHAR(MAX) value reference 
          // into the otl_lob_stream from the otl_stream.
  int n=0;
  while(!lob.eof()){ // read while not "end-of-file" -- end of VARCHAR(MAX) value
   ++n;
   lob>>f2; // reading a chunk of VARCHAR(MAX) value
   cout<<"   chunk #"<<n;
   cout<<", f2="<<f2[0]<<f2[f2.len()-1]<<", len="<<f2.len()<<endl;
  }
  lob.close(); // closing the otl_lob_stream.
  n=0;
  while(!lob2.eof()){ // read while not "end-of-file" -- end of VARCHAR(MAX) value
   ++n;
   lob2>>f2; // reading a chunk of VARCHAR(MAX) value
   cout<<"   chunk #"<<n;
...
int main()
{
 ...
 try{
  ...
  otl_cursor::direct_exec
   (
    db,
    "create table test_tab(f1 int, f2 VARCHAR(MAX), f3 VARCHAR(MAX))"
    );  // create table
