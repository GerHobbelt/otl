...
{ 
 ...
 i.open(1, // buffer size. To read CLOBs, it should be set to 1
 ...
 // while not end-of-data
  it>>f1;
  cout<<"f1="<<f1<<endl;
  it>>lob; // initializing LOB stream by reading the CLOB reference 
          // into the otl_lob_stream from the otl_stream.
  it>>lob2; // initializing LOB stream by reading the CLOB reference 
          // into the otl_lob_stream from the otl_stream.
  int n=0;
...
int main()
{
 ...
 try{
  ...
  otl_cursor::direct_exec
   (
    db,
    "create table test_tab(f1 int, f2 clob(1M), f3 clob(1M))"
    );  // create table
