...
bool otl_check_out_type_func(int bind_var_type,int cpp_var_type);

#define OTL_CHECK_OUT_TYPE_FUNC otl_check_out_type_func

#include <otlv4.h> // include the OTL 4.0 header file

bool otl_check_out_type_func(int bind_var_type,int cpp_var_type)            
{                     
  cout<<"BIND="<<bind_var_type<<", CPP="<<cpp_var_type<<endl;
  if(bind_var_type==otl_var_bigint && cpp_var_type==otl_var_int)
      return true;
  else                                                                  
    return false;                                                       
}
