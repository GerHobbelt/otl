...
{otl_long_string f2(6000); // define long string variable
 otl_stream o(1, // buffer size has to be set to 1 for operations with LOBs
              "insert into test_tab values(:f1<int>,empty_blob(),empty_blob()) "
              "returning f2, f3 into :f2<blob>,:f3<blob> ",
                 // SQL statement
              db // connect object
             );
...
{ 
 ...
 i.open(10, // buffer size. To read BLOBs, it can be set to a size greater than 1
 ...
 int f1;
 otl_lob_stream* lob; // Pointer to Stream for reading BLOB
 otl_lob_stream* lob2; // Pointer to Stream for reading BLOB
 ...
 while(rs.next_row()){ // while not end-of-data
  rs.get(1,f1);
  cout<<"f1="<<f1<<endl;
  rs.get(2,lob); // get pointer to BLOB stream
  rs.get(3,lob2); // get pointer to BLOB stream
  int n=0;
  ...
  lob->close(); // closing the otl_lob_stream.
  n=0;
  while(!lob2->eof()){ // read while not "end-of-file" -- end of BLOB
   ++n;
...
int main()
{
 ...
 try{
  ...
  otl_cursor::direct_exec
   (
    db,
    "create table test_tab(f1 number, f2 blob, f3 blob)"
    );  // create table
