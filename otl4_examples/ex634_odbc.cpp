...
otl_connect db2; // connect object for the second connection which will be
                 // used for selecting "auto-committed" rows
...
{ 
 otl_stream i(50, // buffer size
              "select * from test_tab where f1>=:f11<int> and f1<=:f2<int>*2",
                 // SELECT statement
              db2 // connect object
             ); 
...
int main()
{
 ...
 try{
  ...
  db.rlogon("informix/tigger@informixsql",1); // connect to Informix in the auto-commit mode.
  db2.rlogon("informix/tigger@informixsql"); // connect to Informix
 ...
 db2.logoff(); // disconnect from Informix
