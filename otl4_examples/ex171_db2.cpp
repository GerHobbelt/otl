...
{ 
 otl_stream o(50, // buffer size
              ...
              "values(:f1<float>,:f2<char[31]>,NULL, "
              " :f4<timestamp>, :f5<db2date>, :f6<db2time>)",  
                 // SQL statement
              db // connect object
             );
 char tmp[32];
 otl_datetime f4(2001,2,24,13,21,11);
 otl_datetime f5(2001,2,24,0,0,0);
 otl_datetime f6(0,0,0,13,21,11);
...
#endif
  if(i==10)
    o<<static_cast<float>(i)<<otl_null()<<otl_null()<<otl_null()<<otl_null();
  else
    o<<static_cast<float>(i)<<tmp<<f4<<f5<<f6;
 }
...
{ 
 otl_stream i(50, // buffer size
              "select f1,f2,f2 f22,f3,f4,f5,f6 "
               "from test_tab "
