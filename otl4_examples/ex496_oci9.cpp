...
#define OTL_ORA_UTF8 // Enable UTF8 OTL for OCI8i
...
{ 
 otl_long_string f2(70000); // define long string variable
 otl_long_string f3(70000); // define long string variable
 db.set_max_long_size(70000); // set maximum long string size for connect object
 ...
 while(!i.eof()){ // while not end-of-data
  i>>f1>>f2>>f3;
  cout<<"f1="<<f1;
  cout<<", f2="<<f2[0]<<f2[f2.len()-1]<<", len="<<f2.len();
  cout<<", f3="<<f3[0]<<f3[f3.len()-1]<<", len="<<f3.len();
  cout<<endl;
 }
